# Updated for PySpark 3.5 (2025) — imports, type hints, and minor API tweaks
# ──────────────────────────────────────────────────────────────────────────
...
# Custom helpers assumed to exist in your project
from pyspark import SparkConf, SparkContext          # still valid if used elsewhere
from your_package.spark_session_manager import SparkSessionManager  # adjust to real path
from your_package.logger import Logger                               # adjust to real path


class SCDHandler:
    def __init__(self):
        self.spark: SparkSession = SparkSessionManager(
            self.__class__.__name__
        ).create_session()
        self.logger = Logger(self.__class__.__name__)

    def check_columns_presence(self,source_df: DataFrame,target_df: DataFrame,metadata_cols: List[str],) -> None:
        """
        ...
        """
        cols_missing = (
            {c for c in target_df.columns if c not in source_df.columns}
            - set(metadata_cols)
        )
        if cols_missing:
            raise Exception(f"Cols missing in source DataFrame: {cols_missing}")

    def apply_hash_and_alias(self,source_df: DataFrame,target_df: DataFrame,metadata_cols: List[str],) -> Tuple[DataFrame, DataFrame]:
        """
    ...
    def scd_2(self,source_df: DataFrame,target_df: DataFrame,join_keys: List[str],metadata_cols: List[str] | None = None,) -> DataFrame:
        ...
        source_df, target_df = self.apply_hash_and_alias(
            source_df, target_df, metadata_cols
        )

        # Identify new records
        join_cond = [
            source_df[jk] == target_df[jk] for jk in join_keys
        ]
        ...
        obsolete_df = (
            updated_df.select("target_df.*")
            .withColumn("eff_end_date", current_date())
            .withColumn("flag", lit(0))
        )

        # union : new & updated records and add scd2 meta-data
        delta_df = (
            new_df.unionByName(updated_new_df)
